...
class TestSuite():
    def test_get_files(self, prepare_data):
        ...
        exists_file = list(filter(
            lambda file_n: file_n.get('name') in [test_file_1, test_file_2, test_file_3, test_file_4, test_file_5],
            data
        ))
        exists_files = list(map(lambda file_n:file_n.get('name'), exists_file))
    ...
    def test_get_file_info(self, prepare_data):
        ...
        data = FileServiceNoClass.get_file_data(test_file_part)
    ...
    def test_create_file_with_content(self, prepare_data):
        ...
        data = FileServiceNoClass.create_file(test_content)
        filename = data.get('name')
        assert os.path.exists('{}/{}'.format(test_folder, filename))
    ...
    def test_delete_file(self, prepare_data):
        ...
        data = FileServiceNoClass.delete_file(test_file_part)
