...
from sqlalchemy import create_engine, Column, Integer, DateTime, String, ForeignKey, Boolean
...
class DataBase(metaclass=SingletonMeta):
    ...
    class User(BaseModel, Base):
        ...
        last_login_dt = Column(DateTime, name='LastLoginDate')
        ...
        def __init__(self, email: str, password: str, name: str, surname: str = None, role=None, sessions: list=None) -> None:
            super().__init__()
            self.email = email
    ...
    class Role(BaseModel, Base):
        ...
        def __init__(self, name: str, users: list=None, methods: list=None) -> None:
            super().__init__()
            self.name = name
    ...
    class Method(BaseModel, Base):
        ...
        def __init__(self, name: str, shared: bool=False, roles: list=None) -> None:
            super().__init__()
            self.name = name
            self.shared = shared

            if roles:
                method_role_list = map(lambda role: DataBase.MethodRole(role=role), roles)
                self.roles.extend(method_role_list)

    class MethodRole(Base):
        __tablename__ = 'MethodRole'

        def __init__(self, method=None, role=None) -> None:
            self.method = method
            self.role = role
    ...
    class Session(BaseModel, Base):
        ...
        exp_dt = Column(DateTime, name='ExpirationDate')
    ...
    def init_system(self) -> None:
        ...
        role_admin = self.Role(
            'admin',
            users=[self.User('admin@fileserver.eu', HashAPI.hash_sha512(os.environ['ADMIN_PASSWORD']), 'Admin')]
        )
