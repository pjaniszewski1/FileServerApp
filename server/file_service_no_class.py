...
def get_files():
    ...
    file = [f for f in files if len(f.split(".")) > 1 and f.split(".")[1] == extension]
    for f in files:
        full_filename = "{}/{}".format(path, f)
        data.append({
            "name": f,
            "created_date": utils.convert_date(os.path.getctime(full_filename)),
            "edit_date": utils.convert_date(os.path.getctime(full_filename)),
...
def get_file_data(filename):
    ...
    with open(full_filename, "rb") as file_handler:
        return dict({
            "name": short_filename,
            "create_date": utils.convert_date(os.path.getmtime(full_filename)),
            "edit_date": utils.convert_date(os.path.getmtime(full_filename)),
            "size": os.path.getsize(full_filename),
            "content": file_handler.read().decode("ascii")
        })
